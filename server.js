...
app.post('/api/suggest/secdule/withTimeChecking',function(req,res){
	...
	func.suggestSchedule_prepareData(schedule , function(callbackMonth, callbackDate , callbackTime, callbackLocation){
		...
		var disPath =[];
		var finalizedPath = [];
		func.findObject(location ,data, function(result){//find object for lan,lon from buffered data 
			siteObjectArray = result;
			
			func.calculateEachDistance(siteObjectArray,function(output){//cal distance betweeh each location
				distanceList = output;		
...
console.log("time: " + time[count])
		validateTime(data, month, date, disPath[count], time[count], function(boo){
			if(boo == true){
...
console.log(disPath[count]);
	...
	function validateTime(data, month, date, title, time,callback){
console.log(time);
		var startT;
		var ednT;
		time = toString(time);
		...
		func.checkDayofWeek(month,date,function(ans){
			tarHour = siteObj.openHours;
console.log("tarHour: " +tarHour);
			if( !tarHour[ans].equals("na")){
				startT = tarHour[ans].substring(0,tarHour[ans].indexOf("-"));
				endT = tarHour[ans].substring(tarHour[ans].indexOf("-")+1);
